...
export function photoUI({ root, api, shell, navigate, state, current, confirmDeletion }) {
  ...
  async function settings(version) {
    ...
    root.querySelector('#participant-logout').onclick = async event => {
      event.target.disabled = true;
      try { await api('/api/logout', { method: 'POST' }); await state().refresh(); navigate('/', true); }
      catch (error) { root.querySelector('#message').textContent = error.message; event.target.disabled = false; }
    };
    try {
